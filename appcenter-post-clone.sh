...
if [ -n "$APPCENTER_XCODE_PROJECT" ]; then 
      ...
      echo "Creating AVD..."
      echo "no" | $ANDROID_HOME/tools/bin/avdmanager create avd -n Nexus_5X_API_21_-_GPlay -k "system-images;android-21;google_apis;x86_64" --device "Nexus 5" --force
      ...
      echo "Starting AVD..."
      #some flags to play with: -qemu -no-window -enable-kvm -no-snapshot -noaudio 
      #https://stackoverflow.com/questions/47748948/android-emulator-never-finishes-booting-when-no-window-flag-is-used
      
      nohup $ANDROID_HOME/emulator/emulator -avd Nexus_5X_API_21_-_GPlay -gpu on -no-accel   -no-boot-anim -partition-size 2048 -wipe-data -skin "1080x1920" & #> /dev/null 2>&1 & (uncomment to hide output)
